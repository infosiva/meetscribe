...
export default function FloatingChatWrapper() {
  ...
  const [msgs, setMsgs] = useState<{ role: 'user' | 'bot'; text: string }[]>([
    { role: 'bot', text: 'Hi! Need help with meeting notes or summaries? 📝' },
  ])
        ...
        {open && (
          <motion.div
            ...
            style={{ position: 'fixed', bottom: 88, right: 24, width: 320, height: 420,
              background: 'rgba(8,8,20,0.97)', border: '1px solid rgba(99,102,241,0.3)',
              borderRadius: 16, display: 'flex', flexDirection: 'column', zIndex: 1000,
              overflow: 'hidden', backdropFilter: 'blur(20px)' }}
          >
            <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(99,102,241,0.3)', fontSize: 13, fontWeight: 700, color: '#f8fafc' }}>
              MeetScribe AI
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {msgs.map((m, i) => (
                <div key={i} style={{
                  ...
                  background: m.role === 'user' ? 'rgba(99,102,241,0.3)' : 'rgba(255,255,255,0.06)',
                  padding: '8px 12px', borderRadius: 10, fontSize: 12, color: 'rgba(248,250,252,0.9)', maxWidth: '85%',
                }}>{m.text}</div>
              ))}
            </div>
            <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(99,102,241,0.3)', display: 'flex', gap: 8 }}>
              <input value={input} onChange={e => setInput(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && send()}
                placeholder='Ask about meetings or notes…'
              ...
              <button onClick={send}
                style={{ background: 'linear-gradient(135deg,#6366f1,#4f46e5)', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, color: '#fff', cursor: 'pointer' }}>→</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
